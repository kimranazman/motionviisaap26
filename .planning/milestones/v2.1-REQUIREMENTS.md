# Requirements Archive: v2.1 Navigation & Views
...
- [x] **NAV-01**: User can collapse/expand the SAAP navigation group in the sidebar via a clickable header with chevron icon
- [x] **NAV-02**: User can collapse/expand the CRM navigation group in the sidebar via a clickable header with chevron icon
- [x] **NAV-03**: User can collapse/expand the Admin navigation group in the sidebar (visible to ADMIN role only)
- [x] **NAV-04**: Chevron icon rotates smoothly (CSS transition) when toggling collapse state
- [x] **NAV-05**: Sidebar collapse state persists in localStorage across page navigations and browser sessions
- [x] **NAV-06**: All sidebar groups default to expanded on first visit (before any localStorage value exists)
- [x] **NAV-07**: When user navigates to a route inside a collapsed group, that group auto-expands to show the active item
- [x] **NAV-08**: Mobile sidebar (Sheet) uses the same collapsible groups as the desktop sidebar, sharing a single navigation config
- [x] **NAV-09**: Collapsed group headers show item count badge (e.g., "SAAP (8)")
- [x] **NAV-10**: Sidebar includes top-level "Tasks" link to /tasks and "Members" link to /members (not nested in any group)
- [x] **NAV-11**: Desktop and mobile sidebars share a unified navigation config (eliminates current duplication and fixes missing Price Comparison in mobile)

### Tasks (TASK)

- [x] **TASK-01**: User can navigate to /tasks to see all tasks across all projects
- [x] **TASK-02**: Table view displays tasks with columns: Title, Project, Status, Priority, Assignee, Due Date
- [x] **TASK-03**: Kanban view displays tasks in three columns: To Do, In Progress, Done
- [x] **TASK-04**: User can toggle between Table and Kanban views via a view switcher in the page header
- [x] **TASK-05**: User can filter tasks by assignee (All, Khairul, Azlan, Izyani)
- [x] **TASK-06**: User can filter tasks by project (dropdown of projects that have tasks)
- [x] **TASK-07**: User can filter tasks by status (TODO, IN_PROGRESS, DONE)
- [x] **TASK-08**: User can filter tasks by priority (LOW, MEDIUM, HIGH)
- [x] **TASK-09**: User can search tasks by title (real-time text filter)
- [x] **TASK-10**: User can sort table columns by clicking column headers (ascending/descending toggle)
- [x] **TASK-11**: Each task row/card shows project name as a badge for cross-project context
- [x] **TASK-12**: User can click a task to open a detail sheet showing full task info (title, description, status, priority, assignee, due date, subtasks, comments)
- [x] **TASK-13**: User can drag task cards between kanban columns to change status (reuses existing dnd-kit pattern)
- [x] **TASK-14**: User can filter tasks by due date range (Overdue, Due Today, Due This Week, Due This Month, No Due Date)
- [x] **TASK-15**: Selected view preference (table vs kanban) persists in localStorage

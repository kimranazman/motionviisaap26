...
must_haves:
  truths:
    ...
    - "Project financials show AI-calculated revenue vs costs with profit/loss"
  artifacts:
    ...
    - path: "src/components/projects/project-detail-sheet.tsx"
      provides: "Project detail with financials summary section"
      contains: "FinancialsSummary"
  key_links:
    ...
    - from: "src/components/projects/project-detail-sheet.tsx"
      to: "project.revenue and project.costs"
      via: "calculate and display profit/loss"
      pattern: "revenue.*costs|profit|loss"
---

<objective>
Integrate AI document intelligence into existing UI and add dashboard visibility.

Purpose: Connect all AI components into the existing workflow so users can see pending analysis, trigger reviews, monitor AI-processed documents, and view financial summaries.
Output: AI status badges on documents, pending analysis dashboard widget, manifest auto-generation on upload, and financials summary showing revenue vs costs.
</objective>
...
4. Load AI results when opening review:
   - Fetch from /api/projects/{id}/ai-results (created in Plan 03 Task 4)
...
<task type="auto">
  <name>Task 4: Add Financials Summary section to project detail</name>
  <files>src/components/projects/project-detail-sheet.tsx</files>
  <action>
Add a Financials Summary section to project-detail-sheet.tsx that displays AI-calculated revenue vs costs.

Implementation:
1. Create a FinancialsSummary sub-component within the file (or import if reusable):
   - Takes project object as prop
   - Calculates:
     - Total Revenue: project.revenue (from AI-imported invoices)
     - Total Costs: sum of project.costs[].amount
     - Profit/Loss: revenue - costs
     - Margin %: ((revenue - costs) / revenue * 100) if revenue > 0

2. Display in a dedicated section after Documents section:
   - Section header: "Financials Summary"
   - Grid layout (2 columns on desktop, 1 on mobile):
     - Revenue card: green background, shows total with "From X invoices" subtitle
     - Costs card: red/amber background, shows total with "From X expenses" subtitle
   - Full-width Profit/Loss card below:
     - Green if positive (profit), red if negative (loss)
     - Shows amount and percentage
     - Shows "Break-even" if exactly 0

3. Visual styling:
   - Use shadcn Card components
   - Match existing dashboard widget colors:
     - Revenue: bg-green-50 border-green-200 text-green-700
     - Costs: bg-red-50 border-red-200 text-red-700
     - Profit: bg-green-100 text-green-800
     - Loss: bg-red-100 text-red-800
   - Use formatCurrency utility for amounts
   - Icons: TrendingUp for profit, TrendingDown for loss (from lucide-react)

4. Empty state:
   - If no revenue and no costs: "No financial data yet. Import invoices and receipts to see summary."
   - If only revenue: Show revenue, costs as MYR 0, and profit = revenue
   - If only costs: Show revenue as MYR 0, costs, and loss = -costs

5. Ensure project query includes costs with their amounts for calculation.
  </action>
  <verify>
Project detail sheet shows Financials Summary section.
Revenue displays total from project.revenue field.
Costs displays sum of all project costs.
Profit/Loss calculated correctly with appropriate color coding.
Section renders cleanly with no data (empty state).
  </verify>
  <done>Project detail sheet displays AI-calculated revenue vs costs with profit/loss, fulfilling AI-08 requirement.</done>
</task>
...
7. Financials Summary shows revenue vs costs with profit/loss calculation
8. Financials display handles edge cases (no data, only revenue, only costs)
</verification>
...
- Project financials show revenue vs costs with clear profit/loss indication
</success_criteria>
